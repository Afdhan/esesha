...
cd /etc/openvpn/
wget https://github.com/Afdhan/esesha/raw/main/vpn.zip
unzip vpn.zip
rm -f vpn.zip
...
mkdir /root/OpenVPN
cp /etc/openvpn/client-tcp-ssl.ovpn OpenVPN/Client
cp /etc/openvpn/client-udp-2200.ovpn OpenVPN/Client
cp /etc/openvpn/client-tcp-1194.ovpn OpenVPN/Client
cd /root
zip -r all-ovpn.zip OpenVPN > /dev/null 2>&1
